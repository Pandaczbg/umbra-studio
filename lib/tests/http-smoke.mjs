...
const report = {
  ...
  issues: [],
  browser:
    "HTTP checks only; separate V10 browser report documents actual layout and interaction checks.",
};
const pages = new Map();
...
try {
  ...
  for (const path of ["/blog/not-an-approved-post", "/en/blog/not-an-approved-post"]) {
    const response = await fetch(origin + path);
    assert(response.status === 404, `${path}: unknown article must be 404`);
    report.checks.push({path, status: response.status});
  }
  for (const path of ["/books/Mrzim-svog-brata/mrzim-svog-brata-sr.pdf", "/books/Mrzim-svog-brata/mrzim-svog-brata-en.pdf"]) {
    const response = await fetch(origin + path);
    assert(response.status === 404, `Private source is still public: ${path}`);
    report.checks.push({path, status: response.status});
  }
  const account = await fetch(origin + "/api/account");
  const accountData = await account.json();
  assert(accountData.enabled === false && accountData.user === null, "Unconfigured account must be honestly unavailable");
  report.checks.push({path: "/api/account", status: account.status});
  const sources = await fetch(origin + "/api/search?q=Biblija&locale=sr");
  const sourceResults = await sources.json();
  assert(sourceResults.results.some((r) => r.kind === "source" && r.href.endsWith("#source")), "Source search missing");
  report.checks.push({path: "/api/search?q=Biblija", status: sources.status});
